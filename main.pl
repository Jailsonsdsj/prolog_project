...
init_dynamic_facts:-
  ...
  assertz(turned_off(shower)),      % verifica o chuveiro
  assertz(turned_off(stomach)),     % verifica se o player ja comeu
...
get_in_shower(shower).      % predicado que vai nos permitir ligar o chuveiro sem dar take nele
...
eco(_):-
  not(turned_on(shower)),!.    % se nao tiver ligado entao True e o player pode sair
eco(_):-
  write('You can''t go out with the shower turned on'),nl,!,fail.    % se tiver ligado fail e o player precisa desligar para sair
goto(_):- look.
...
eat2(Thing):-
  turned_on(stomach),     % verifica se o player ja comeu uma vez
  edible(Thing),
  retract(have(Thing)),
  respond(['That ',Thing,' was good']).
eat2(Thing):-
  turned_off(stomach),     % verifica se o player nao comeu ainda
  edible(Thing),
  retract(have(Thing)),    
  retract(turned_off(stomach)),     % retira o predicado turned_off de stomach
  asserta(turned_on(stomach)),      % coloca o turned_on
  respond(['That ',Thing,' was good']).  % player comeu
eat2(Thing):-
  tastes_yuchy(Thing),
...
turn_on(Thing):-
  ...
  turn_on2(Thing).
turn_on(Thing):-
  get_in_shower(Thing),     % se a Thing for um shower 
  here(bathroom),           % pessoa tem que estar no banheiro
  turn_on2(Thing).
turn_on(Thing):-
  get_in_shower(Thing),    % se for um shower e a pessoa nao estiver no banheiro
  respond(['You are not in the bathroom']).
turn_on(Thing):-
  respond(['You don''t have the ',Thing]).
...
% turn_off - yes now i did it

turn_off(Thing):-    % basicamento o mesmo do turn_on mas o inverso
  have(Thing),
  turn_off2(Thing).
turn_off(Thing):-
  get_in_shower(Thing),
...
puzzle(goto(cellar)):-
  have(flashlight),
  turned_on(stomach),        % tem que estar com o stomach on ou seja ter comido
  turned_on(flashlight),!.
puzzle(goto(cellar)):-
  turned_on(stomach),        % ter comido mas nao ta com a lanterna ligada
  turned_off(flashlight),
  write('You can''t go to the cellar because it''s dark in the'),nl,
  write('cellar, and you''re afraid of the dark.'),nl,
  !,fail.
puzzle(goto(cellar)):-
  turned_off(stomach),        % nao ter comido nem ter a lanterna ligada
  turned_off(flashlight),
  write('You''re too hungry to take the stairs and its too dark'),nl,!,fail.
puzzle(goto(cellar)):-
  turned_off(stomach),        % lanterna ligada mas nao ter comido
  turned_on(flashlight),
  write('You''re too hungry to take the stairs'),nl,!,fail.
puzzle(_).
